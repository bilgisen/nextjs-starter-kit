 ...
 */
async function testOAuthLogin(): Promise<{ user: User; cookies: Record<string, string> }> {
  console.log('\n=== Testing OAuth Login ===');
  
  try {
    // First, try to get the current session to see if we're already authenticated
    console.log('\nStep 1: Checking current session...');
    const initialSession = await makeRequest(`${API_URL}/auth/session`, {
      method: 'GET',
      includeCookies: true
    });
    
    const sessionData = initialSession.data as Session;
    
    if (sessionData?.user) {
      console.log('✅ Already authenticated as:', sessionData.user.email);
      return {
    ...
    await makeRequest(`${API_URL}/auth/social/google`, {
      method: 'GET',
      queryParams: {
    ...
    while (attempts < maxAttempts) {
      ...
      sessionResponse = await makeRequest(`${API_URL}/auth/session`, {
        method: 'GET',
        includeCookies: true
      });
      
      const sessionData = sessionResponse.data as Session;
      if (sessionData?.user) {
 ...
 */
async function testJwtToken(): Promise<{ token: string; verified: boolean }> {
...
async function runTests() {
  ...
  try {
    // Test 1: OAuth Login
    const { user } = await testOAuthLogin();
    console.log(`✅ Logged in as: ${user.email}`);
    
    // Test 2: JWT Token
    const tokenResult = await testJwtToken();
